...
| **飘字系统** | 屏幕空间伤害数字，支持元素色/自定义色/渐变色、缩放动画、上升淡出，5种类型(NORMAL/CRIT/REACTION/HEAL/TEXT) |

### 技术栈

- **Minecraft 版本**: NeoForge 26.2
- **Java 版本**: 21
- **依赖库**: LowDragLib2（同步框架、RPC 网络、ModularUI HUD）
...
| `artifact.toml` | 圣遗物主副词条配置 |
| `monster_level.toml` | 怪物等级生成配置 |
| `damage_indicator.toml` | 伤害飘字颜色配置 |
...
```
服务端 LivingEntityHurtMixin
  └─ DamageIndicatorFactory.damage(target, modSource, finalDamage, element)
        ├─ 计算 origin（攻击者身体与目标之间 0.5 位置）
        ├─ 计算 finalPos（目标上方 + 随机散布）
        ├─ DamageIndicatorConfig.getColorForElement(element)
        └─ for 附近 48 格玩家
              └─ DamageIndicatorRpc.sendToPlayer(...)

RPC：RPCPacketDistributor.rpcToPlayer(player, "genshin:damage_indicator", 全参数)
  │
  ▼
客户端 DamageIndicatorRpc.damageIndicatorRPCPacket(sender, ...)
  └─ if (sender.isServer())
        └─ new DamageIndicator(...) → DamageIndicatorManager.add(...)
...
伤害飘字系统在实体受伤时，在屏幕空间（HUD 层）显示一个浮动的伤害数字。采用 **服务端 → 客户端 RPC 单向下发** 的设计：服务端计算好最终伤害后，通过 LDLib2 RPC 把数据发给附近玩家，客户端用 LDLib2 的 `ModularHudLayer` 把数字画在屏幕上。

**为什么不用世界空间渲染**：NeoForge 26.2 的 `SubmitNodeCollector` 只在 `SubmitCustomGeometryEvent` 阶段对纯几何体（`submitCustomGeometry`）有效；`submitText` / `submitNameTag` 在这个阶段无法生效（引擎不会在此阶段处理文本渲染管线）。所以改用 HUD 2D 绘制，并通过 `projectPointToScreen` 把世界坐标投影到屏幕坐标。

**两层架构**：

| 层 | 包 | 职责 |
|----|----|------|
| **服务端层** | `core.system.combat.damage` | `DamageIndicatorFactory`（入口）、`DamageIndicatorRpc`（RPC 发送） |
| **客户端层** | `client.damage` | `DamageIndicator`（动画状态）、`DamageIndicatorManager`（活跃列表）、`DamageIndicatorRenderer`（HUD 更新）、`ClientHudRegistration`（HUD 层注册） |

依赖方向：服务端层通过 RPC 把数据传给客户端层，客户端层读取动画状态并把数字画到 HUD。

#### 关键文件

| 类 | 路径 |
|----|------|
| 元素颜色配置 | `config/DamageIndicatorConfig.java` |
...
| HUD 注册 | `client/damage/ClientHudRegistration.java` |
...
`DamageIndicatorFactory` 是所有飘字的**统一入口**，提供 5 类 × 3 种颜色模式 = 20+ 个便捷方法。

##### 颜色模式

| 模式 | 后缀 | 说明 |
|------|------|------|
| **Default** | 无后缀 | 按元素/反应类型从 `damage-indicator.toml` 取色 |
| **Custom** | `Custom` | 传入单个十六进制颜色（`int color`） |
| **Gradient** | `Gradient` | 传入顶部+底部两个颜色（`int topColor, int bottomColor`），客户端做垂直渐变 |

##### 五种类型

| 方法族 | Style枚举 | 用途 | 颜色来源 |
|--------|----------|------|----------|
| `damage` / `damageCustom` / `damageGradient` | `NORMAL` | 普通伤害数字 | 元素色 / 自定义 / 渐变 |
| `crit` / `critCustom` / `critGradient` | `CRIT` | 暴击伤害数字 | 元素色 / 自定义 / 渐变 |
| `reaction` / `reactionCustom` / `reactionGradient` | `REACTION` | 元素反应名称 | 反应色 / 自定义 / 渐变 |
| `heal` / `healCustom` / `healGradient` | `HEAL` | 治疗数字 | 固定草元素色 / 自定义 / 渐变 |
| `text` / `textCustom` / `textGradient` | `TEXT` | 纯文本/免疫等消息 | 白色(物理色) / 自定义 / 渐变 |
...
// 暴击（默认外观 + .crit class）
DamageIndicatorFactory.crit(target, modSource, finalDamage, element);

// 元素反应（ElementalReactionType 枚举，自动取反应色）
DamageIndicatorFactory.reaction(target, ElementalReactionType.VAPORIZE);

// 元素反应（带攻击者，从攻击者与目标中间飘出）
DamageIndicatorFactory.reaction(target, attacker, ElementalReactionType.OVERLOAD);

// 治疗
DamageIndicatorFactory.heal(target, healAmount);

// 纯文本
DamageIndicatorFactory.text(target, "免疫");

// 自定义单色伤害（金色）
DamageIndicatorFactory.damageCustom(target, source, finalDamage, 0xFFFFAA00);

// 渐变伤害（顶部金色、底部红色）
DamageIndicatorFactory.damageGradient(target, source, finalDamage, 0xFFFFAA00, 0xFFFF3333);

// 自定义外观（Options）
DamageIndicatorFactory.damage(target, source, finalDamage, element,
        DamageIndicatorFactory.Options.of(2.5f, 5.0f, 1200L));
...
`Options` 是本次飘字的**可配置外观参数**，3 个字段全部有默认值：

| 字段 | 类型 | 默认值 | 作用 |
|------|------|--------|------|
| `baseScale` | `float` | `2.2` | 稳定后的基础大小倍率。想让字整体更大就调高 |
| `startScale` | `float` | `6.2` | 出现瞬间的大小倍率。应 > `baseScale`，形成"由大变小"动画 |
| `durationMs` | `long` | `950` | 总持续时间（毫秒）。影响上升阶段时长和淡出时刻 |

**构造方式**：

```java
// 完全指定
new Options(3.0f, 7.0f, 1200L)

// 快捷静态方法
Options.of(3.0f, 7.0f, 1200L)
...
`DamageIndicator` 内部把 `durationMs` 拆成两段：
...
0ms ─────────── 200ms ───────────────────── 650ms ────── 950ms
 ...
 │  200ms 完成    │  整个生命周期都在上升     │  300ms    │
 │  缩放 6.2→2.2  │  上升总高度 RISE_HEIGHT  │  1→0      │
 │  easeOutCubic  │                          │           │
```
...
| 常量 | 值 | 说明 |
|------|-----|------|
| `MOVE_MS` | `200` | 飞向落点的时间。短一点，快速到位 |
| `FADE_MS` | `300` | 结束淡出时长 |
| `RISE_HEIGHT` | `0.23` | 整个生命周期上升的总高度（格）。**这就是"上升速度"的调节旋钮** |

**缩放阶段**：从 `startScale` 缩到 `baseScale`，只在 `MOVE_MS` 内发生。使用 **easeOutCubic** 缓动曲线（`1 - (1-t)³`），视觉上快速缩到目标大小再缓慢收尾，模拟"冲击→稳定"的弹性感。

**上升阶段**：**从出生起就持续上升**（线性匀速），不是到达落点才开始。这样视觉上没有"到落点停顿再上升"的割裂感。

**REACTION 额外偏移**：反应名称飘字额外抬高 `+0.2`（Y轴），与伤害数字垂直错开避免重叠。

#### 如何调整动画

| 想改什么 | 改哪里 |
|---------|--------|
| **字整体更大** | 传更大的 `baseScale`（如 `Options.builder().baseScale(2.8f)`） |
| **出场更夸张** | 传更大的 `startScale`（如 `7.5f` / `9.0f`） |
| **上升更快** | `DamageIndicator.RISE_HEIGHT` 调大（如 `0.4`） |
| **上升更慢** | `DamageIndicator.RISE_HEIGHT` 调小（如 `0.15`） |
| **整个动画时长** | 传不同的 `durationMs`（如 `1500L`，配合 `RISE_HEIGHT` 不变则上升速度不变） |
| **飞向落点更快** | `DamageIndicator.MOVE_MS` 调小（如 `150`） |
| **飞向落点更慢** | `DamageIndicator.MOVE_MS` 调大（如 `300`） |
| **淡出更慢** | `DamageIndicator.FADE_MS` 调大（如 `500`） |

**上升速度公式**：`速度 = RISE_HEIGHT / durationMs`。比如 `RISE_HEIGHT = 0.23`、`durationMs = 950`，则速度约 `0.00024 格/ms`，即 `0.24 格/秒`。

#### 如何调整距离与视野

`DamageIndicatorRenderer` 顶部有一组距离常量：

| 常量 | 默认 | 说明 |
|------|------|------|
| `NEAR_DISTANCE` | `4.0` | 相机距离 ≤ 此值时，字保持原大小 |
| `FAR_DISTANCE` | `32.0` | 相机距离 ≥ 此值时，字缩到 `FAR_SCALE` |
| `FAR_SCALE` | `0.25f` | 远端的缩放下限 |
| `MAX_RENDER_DISTANCE` | `128.0` | 超过这个距离直接不绘制 |

**缩放曲线**：`[NEAR_DISTANCE, FAR_DISTANCE]` 区间内做**分段线性**，`1.0 → FAR_SCALE` 单调递减。**玩家远离时字只会变小，绝不会变大**。

`DamageIndicatorFactory` 还有一个广播半径常量：

| 常量 | 默认 | 说明 |
|------|------|------|
| `BROADCAST_RADIUS` | `48.0` | 只把飘字 RPC 发给此半径内的玩家 |

想改广播范围：调整 `DamageIndicatorFactory.BROADCAST_RADIUS`。

#### 服务端定位逻辑

`DamageIndicatorFactory.spawn()` 内部计算两个世界坐标：

**originPos（出现位置）**：

```java
Vec3 attackerCenter = attacker.position().add(0, attacker.getBbHeight() * 0.7, 0);
// 0.5 = 攻击者身体与目标的中间点
// 0.4 = 偏向攻击者；0.6 = 偏向目标
Vec3 originPos = attackerCenter.lerp(targetCenter, 0.5);
```

- `attacker.position()` 拿的是**玩家身体位置**（脚底），加 `getBbHeight() * 0.7` 得到胸口。
- 第三人称模式下**不会从摄像头冒出**，永远从玩家身体方向出发。
- 若无攻击者（反应、自然伤害），`originPos` 退化为 `targetCenter.add(0, 0.6, 0)`。

**finalPos（最终落点）**：

```java
Vec3 targetCenter = target.position().add(0, target.getBbHeight() * 0.85, 0);
Vec3 finalPos = targetCenter.add(
        (rand.nextDouble() - 0.5) * 0.55,   // X 随机散布 ±0.275
        0.30 + rand.nextDouble() * 0.20,     // Y 抬高 0.30 ~ 0.50
        (rand.nextDouble() - 0.5) * 0.55    // Z 随机散布 ±0.275
);
```

- 每次命中落点都会**随机散布**，避免连续命中数字完全重叠。
- 想让数字更集中：把 `0.55` 改小（如 `0.3`）。
- 想让数字出现更高：把 `0.30 + rand.nextDouble() * 0.20` 的上限调高。

#### 颜色配置

配置文件：`./config/minegenshin/damage-indicator.toml`（注意是连字符 `-` 不是下划线），分两大组：

`DamageIndicatorConfig` 提供两个API：
```java
...
**类元素自动归并**：FROZEN（冻）→ CYRO，AGGRAVATE（激）→ DENDRO，BURNING（燃）→ PYRO。`getColorForElement` 内部调用 `getMainElement()` 自动归并。

#### LSS 样式表

**路径**：`resources/assets/minegenshin/lss/hud/damage_indicator.lss`

样式表控制**与具体伤害无关的通用美化**，颜色由代码动态设置：

```lss
#damage-indicator-root {
    clip: none;
}
.damage-indicator {
}
.damage-indicator.crit {
}
.damage-indicator.reaction {
}
...
文字颜色通过 `Component.withStyle(Style.EMPTY.withColor(TextColor.fromRgb(rgb)))` 设置，**不通过 LSS**。

#### 添加新类型飘字

假设要添加"护盾吸收伤害"飘字：

**Step 1**：在 `DamageIndicatorFactory.Style` 里加一个类型
...
**Step 2**：在 `DamageIndicatorFactory` 加便捷方法

```java
public static void shield(LivingEntity target, float absorbed) {
    shield(target, absorbed, Options.DEFAULT);
}

public static void shield(LivingEntity target, float absorbed, Options options) {
    // 数值类飘字：用 spawnValue 取整
    spawnValue(target, absorbed, 0xFFBBCCDD, 0xFFBBCCDD, Style.SHIELD, options);
}
```

**Step 3**：在渲染器里加 class 分支

```java
case 5 -> label.addClass(CLASS_SHIELD);
```

**Step 4**：LSS 里加样式

```lss
.damage-indicator.shield {
}
```
...
```java
DamageIndicatorFactory.shield(target, absorbedAmount);
```

#### 常见问题

**Q：飘字看不见？**
1. 检查 `ClientHudRegistration` 是否注册成功（看日志有没有 `[DI-Hud] hudRoot built`）
2. 检查 `minegenshin.mixins.json` 里 `LivingEntityHurtMixin` 是否列出
3. 检查 `LivingEntityHurtMixin` 里有没有调用 `DamageIndicatorFactory.damage(...)`
4. 看日志有没有 `[DI-RPC] received packet`（客户端收到）

**Q：飘字卡顿？**
- 位置更新用 `label.transform(t -> t.translate(x, y).scale(s))`，**不要**每帧调 `label.layout(...)`。前者不触发 Taffy 重排，后者会。
- `DamageIndicatorRenderer.updateHudLabels()` 开头加 `if (LABEL_ENTRIES.isEmpty()) return;`，无飘字时零开销。

**Q：飘字颜色不对？**
- 检查 `damage-indicator.toml` 里的十六进制颜色值
- 检查元素是不是类元素（冻/激/燃），会自动归并到主元素

**Q：玩家远离时飘字变大？**
- 这是早期版本的 bug，现已用**分段线性距离缩放**修掉。如果你改过 `computeDistanceScale`，确保它是**单调递减**的：
  ```java
...
**Q：飘字从摄像头方向出现？**
- `DamageIndicatorFactory.spawn()` 里用了 `attacker.position()`（玩家身体），不是 `player.getEyePosition()`（眼睛/摄像头）。如果你想更贴胸口，`getBbHeight() * 0.7` 可以调成 `0.85`。
...
| 2 | `LivingEntityHurtMixin` | `LivingEntity.hurtServer()` | HEAD, cancellable | 非 TeyvatLivingEntity 实体的 `ModDamageSource` 伤害入口：调 `HurtEntityHelper.calculateFinalModDamage`，扣血，**调用 `DamageIndicatorFactory.damage(...)` 触发飘字**，广播，cancel |
...
| 飘字颜色配置 | `config/DamageIndicatorConfig.java` |
| 飘字工厂 | `core/system/combat/damage/DamageIndicatorFactory.java` |
| 飘字 RPC | `core/network/DamageIndicatorRpc.java` |
| 飘字动画状态 | `client/damage/DamageIndicator.java` |
| 飘字活跃列表 | `client/damage/DamageIndicatorManager.java` |
| 飘字 HUD 渲染 | `client/damage/DamageIndicatorRenderer.java` |
| 飘字 HUD 注册 | `client/damage/ClientHudRegistration.java` |
| 飘字样式表 | `resources/assets/minegenshin/lss/hud/damage_indicator.lss` |
