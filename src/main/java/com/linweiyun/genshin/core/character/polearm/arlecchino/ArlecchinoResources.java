 ...
 * {@link CharacterActionData#fallback(int) 通用兜底表}，兜底表的普攻/战技/大招现在能跑，
 * 但它<b>不知道这个角色是近战还是远程</b>（每一段都用默认近战形态）、
 * 也不带角色自己的动画名。接上本表之后，动作时序与索敌形态都归角色自己管，
 * 客户端动画与服务端伤害仍然只读同一份数据（见 {@code ResourceDrivenActionHandler}）。
 *
 * <h2>数值口径</h2>
 * <b>时序完全复用兜底表</b>（每一段的 duration / protectDuration / hits / comboWindow
 * 都取自 {@link CharacterActionData#fallback(int)}），本表只做两件事：
 * <ol>
 *   <li>把连段数固定成 {@code ArlecchinoSkill.getMaxCombo()} 声明的 {@value #MAX_COMBO} 段；</li>
 *   <li>把每一段标成<b>长柄近战</b>形态（{@link Engagement#melee()} + 3 格攻击距离）。</li>
 * </ol>
 * 伤害倍率仍然只在 {@link ArlecchinoSkill} 里，本表不管数值。
 */
public final class ArlecchinoResources {

    private ArlecchinoResources() {
    }

    /**
     * 连段数 —— 必须和 {@link ArlecchinoSkill#getMaxCombo()} 一致
     ...
     *
     * <p>长柄是<b>近战</b>，不需要 {@code withAcquireRange}：默认的「索敌 6 格 / 保持 9 格、
     * 差额交给突进补」就是长柄该有的手感（对比法器角色要显式
     * {@code ranged() + withAttackRange(13)}，见 {@code VodyanitsaResources.ATTACK_RANGE} 的注释 ——
     * 索敌半径和生效攻击距离是两件事）。
     */
